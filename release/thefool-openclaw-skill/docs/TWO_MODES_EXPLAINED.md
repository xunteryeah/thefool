...
### 方案 1：Resident Runner 常驻模式 ⭐

启动一个持续运行的常驻进程，由 runner 持有 Socket 连接，并把实时事件送给 brain：

```bash
pm2 start "skills/alicization-town/agent-runner.js" --name thefool-qianzi
```
...
- ✅ 自动重连与状态恢复
- ✅ 持续接收所有实时事件
- ✅ 可接 OpenClaw/QClaw 作为真正 brain
- ✅ 可通过 control plane 做 wakeup / takeover / release / sync
