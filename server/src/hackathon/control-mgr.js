...
class ControlMgr {
  constructor(io) {
    ...
    this.runnerStatusByAgent = new Map();
  }
  ...
  register(socket) {
    ...
    const runnerStatus = this.runnerStatusByAgent.get(agentId);
    if (runnerStatus) {
      socket.emit('runner:status', runnerStatus);
    }
  }
  ...
  updateRunnerStatus(socket, data = {}) {
    const agentId = socket.identity && socket.identity.agentId;
    if (!agentId) return;
    const status = {
      agentId,
      mode: typeof data.mode === 'string' ? data.mode : 'fallback',
      connected: Boolean(data.connected),
      brainEnabled: Boolean(data.brainEnabled),
      brainAttached: Boolean(data.brainAttached),
      brainHealthy: Boolean(data.brainHealthy),
      lastBrainResponseAt: Number(data.lastBrainResponseAt || 0),
      lastBrainAttachAt: Number(data.lastBrainAttachAt || 0),
      act: Number(data.act || 0),
      roomId: typeof data.roomId === 'string' && data.roomId ? data.roomId : null,
      reason: typeof data.reason === 'string' ? data.reason : 'heartbeat',
      time: Number(data.time || Date.now()),
    };
    this.runnerStatusByAgent.set(agentId, status);
    this.io.to(this._room(agentId)).emit('runner:status', status);
    this.io.to('director-room').emit('runner:status', status);
  }
