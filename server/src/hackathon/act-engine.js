...
class ActEngine {
  ...
  setAct(actNumber) {
    ...
    if (num === 1) {
      this._speakerOrder = [...HACKATHON_CONFIG.ACT1_SCRIPT_ORDER];
      this._speakerIndex = -1;
  ...
  _startAct1ScriptedSequence() {
    ...
    this._currentScene = {
      ...
      phase: 'opening',
      background: 'white',
      focusZoom: HACKATHON_CONFIG.ACT_STAGE_SCENE.focusZoom,
    };
    this.io.emit('act:scene', this._currentScene);
    this._schedule(() => this._runAct1Speaker(runId), 600);
  }
  ...
  async _runAct1Speaker(runId) {
    ...
    const intro = HACKATHON_CONFIG.ACT1_INTROS[agentId] || {
      title: agentId,
      text: `${agentId} 登台发言。`,
    };
    const stage = HACKATHON_CONFIG.ACT_STAGE_SCENE.center;
    ...
    }, HACKATHON_CONFIG.ACT_STAGE_SCENE.settleMs);

    this._schedule(() => {
      if (!this._isRunActive(runId)) return;
      if (this._speakerOrder && this._speakerIndex < this._speakerOrder.length - 1) {
      ...
      }, HACKATHON_CONFIG.ACT_STAGE_SCENE.closingHoldMs);
    }, HACKATHON_CONFIG.ACT_STAGE_SCENE.settleMs + HACKATHON_CONFIG.ACT_STAGE_SCENE.introHoldMs);
  }
