...
function isAdminApiKey(apiKey) {
  ...
  return Boolean(entry && entry.role === 'admin' && entry.enabled !== false);
}

app.get('/admin.html', (req, res) => {
  const apiKey = req.headers['x-api-key'] || req.query.key || req.query.apiKey;
  if (!isAdminApiKey(apiKey)) {
    return res.status(401).send('需要管理员权限');
  }
  return res.sendFile(path.join(WEB_ROOT, 'admin.html'));
});
...
// 初始化选手属性
if (playerMgr) {
  playerMgr.init(HACKATHON_CONFIG.AGENT_PLAYER_IDS.length ? HACKATHON_CONFIG.AGENT_PLAYER_IDS : ['qianzi', 'paopao', 'jiajia']);
}
...
// ── 启动服务器 ───────────────────────────────────────────────────────────────
server.listen(PORT, () => {
  console.log(`🔐 已加载 ${HACKATHON_CONFIG.PARTICIPANTS.length} 个 API Key: ${HACKATHON_CONFIG.PARTICIPANTS_FILE}`);
  console.log(`🌍 Hackathon Platform 已启动: http://localhost:${PORT}`);
});
